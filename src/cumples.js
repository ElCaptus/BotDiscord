...
var dataCumples = fs.readFileSync(`${__dirname}/../cumpleanios/cumples.json`,);
...
const formatDB = function (){
    //escribe en
    fs.writeFile(`${__dirname}/../cumpleanios/cumples.json`,dataVirgen,function(err){
        if(err){
            console.log(err);
        }
...
const addCumple = function (st){
    ...
    if((st.length < 5) && (st.length > 1)){
        ...
        fs.writeFile(`${__dirname}/../cumpleanios/cumples.json`,dataCumples,function(err){
            if(err){
...
const toString = function (msg){
    ...
    var cumples_actual = JSON.parse(fs.readFileSync(`${__dirname}/../cumpleanios/cumples.json`));
